...
import scipy
import torch
from monofrd.ops.iou3d_nms.iou3d_nms_utils import boxes_iou_bev
...
def boxes3d_to_grid3d_kitti_camera(boxes3d, size=28, bottom_center=True, surface=False):
    ...
    """
    l, h, w = boxes3d[:, 3], boxes3d[:, 4], boxes3d[:, 5]
    x_corners, y_corners, z_corners = np.meshgrid(np.linspace(-0.5, 0.5, size), np.linspace(-0.5, 0.5, size),
