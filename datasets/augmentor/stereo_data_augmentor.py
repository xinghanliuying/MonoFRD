...
import numpy as np
from monofrd.utils import common_utils, box_utils
...
class StereoDataAugmentor(object):
    def __init__(self, root_path, augmentor_configs, class_names, logger=None):
        ...
        if augmentor_configs is not None:
            aug_config_list = augmentor_configs if isinstance(augmentor_configs,
                                                              list) else augmentor_configs.AUG_CONFIG_LIST
    ...
    def filter_truncated(self, data_dict, config=None):
        ...
        if area_ratio_threshold is not None:
            ...
            gt_boxes_cam = box_utils.boxes3d_lidar_to_kitti_camera(data_dict['gt_boxes'][data_dict['gt_boxes_mask']],
                                                                   None, pseudo_lidar=True)

            boxes2d_image, _ = box_utils.boxes3d_kitti_camera_to_imageboxes(gt_boxes_cam, calib, image_shape,
                                                                            return_neg_z_mask=True, fix_neg_z_bug=True)
            truncated_ratio = 1 - box_utils.boxes3d_kitti_camera_inside_image_mask(gt_boxes_cam, calib, image_shape,
                                                                                   reduce=False).mean(-1)
        ...
        if area_2d_ratio_threshold is not None:
            ...
            boxes2d_image, no_neg_z_valids = box_utils.boxes3d_kitti_camera_to_imageboxes(
                box_utils.boxes3d_lidar_to_kitti_camera(data_dict['gt_boxes'][data_dict['gt_boxes_mask']],
                                                        data_dict['calib'], pseudo_lidar=True),
                data_dict['calib'],
        ...
        if not all(valid_mask):
            ...
            print(config)
            print('filter truncated ratio:',
                  truncated_ratio[invalid_mask] if area_ratio_threshold is not None else 'null',
