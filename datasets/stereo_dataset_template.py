...
from monofrd.ops.roiaware_pool3d import roiaware_pool3d_utils
from monofrd.utils import common_utils, box_utils, depth_map_utils
from monofrd.utils.calibration_kitti import Calibration
...
class StereoDatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, logger=None):
        ...
        if self.dataset_cfg.get("STEREO_VOXEL_SIZE", None):
            ...
            stereo_grid_size = (
                                       self.point_cloud_range[3:6] - self.point_cloud_range[0:3]) / np.array(
                self.stereo_voxel_size)
    ...
    def prepare_data(self, data_dict):
        ...
        if 'points' in data_dict:
            ...
            data_dict['depth_gt_img'] = depth_map_utils.points_to_depth_map(rect_points, image_shape,
                                                                            data_dict['calib'])
            data_dict['depth_gt_right_img'] = depth_map_utils.points_to_right_depth_map(rect_points, image_shape,
                                                                                        data_dict['calib'])
        # import matplotlib.pyplot as plt
        # plt.subplot(3, 1, 1)
        # plt.imshow(data_dict['depth_gt_img'])
        # plt.subplot(3, 1, 2)
        # plt.imshow(data_dict['left_img'])
        # plt.subplot(3, 1, 3)
        # plt.imshow(data_dict['depth_gt_right_img'])
        # plt.show()
        if 'gt_boxes' in data_dict:
            data_dict['depth_fgmask_img'] = roiaware_pool3d_utils.depth_map_in_boxes_cpu(
                data_dict['depth_gt_img'], data_dict['gt_boxes'][:, :7], data_dict['calib'], expand_distance=0.,
                expand_ratio=1.0)
    ...
    def collate_batch(batch_list, _unused=False):
        ...
        for key, val in data_dict.items():
            ...
            elif key in ['left_img', 'right_img', 'depth_gt_img', 'depth_gt_right_img', 'depth_fgmask_img']:
                if key in ['depth_gt_img', 'depth_fgmask_img', 'depth_gt_right_img']:
            ...
            elif key in ['gt_boxes', 'gt_boxes_no3daug', 'gt_boxes_2d', 'gt_centers_2d', 'gt_boxes_2d_ignored',
                         'gt_boxes_camera']:
