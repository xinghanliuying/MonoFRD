...
from monofrd.ops.roiaware_pool3d.roiaware_pool3d_utils import points_in_boxes_gpu
from monofrd.utils import box_coder_utils, common_utils, loss_utils
from monofrd.utils.common_utils import dist_reduce_mean
...
class AnchorHeadTemplate(nn.Module):
    ...
    def get_box_reg_layer_loss(self):
        ...
        box_preds_sin, reg_targets_sin = self.box_coder.process_before_loss(anchors[pos_inds], box_preds[pos_inds],
                                                                            box_reg_targets[pos_inds])
    ...
    def get_imitation_reg_layer_loss(self, features_preds, features_targets, imitation_cfg):
        ...
        if imitation_cfg["mode"] == "inbox":
            ...
            anchors_xyz[..., 2] = 0
            gt_boxes[..., 2] = 0
            # positives = points_in_boxes_gpu(anchors_xyz.view(2*anchors_xyz.shape[0], -1, 3), gt_boxes)
            ...
            positives = features_preds.new_ones(*features_preds.shape[:-1]).bool()
            if dist.get_rank() == 0:
                print("using full imitation mask")
        else:
            raise ValueError("wrong imitation mode")

        if len(features_targets.shape) == 5 and imitation_cfg["mode"] == "inbox":
            # 3d feature
