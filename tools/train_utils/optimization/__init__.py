...
#         return max(cur_decay, optim_cfg.LR_CLIP / optim_cfg.LR)
    def lr_lbmd(cur_epoch):
        ...
        for i, decay_step in enumerate(decay_steps):
            if cur_epoch >= decay_step:
                cur_decay = optim_cfg.LR_DECAY[i]  # 使用对应的衰减值
        return cur_decay
...
#     T_max = total_epochs * total_iters_each_epoch/2
#     lr_scheduler = CosineAnnealingLR(optimizer, T_max, eta_min=optim_cfg.LR / optim_cfg.DIV_FACTOR)
